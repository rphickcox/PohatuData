...
parents <- nests %>%
  ...
  group_by(NestYear, `Nest ID`) %>%
  pivot_longer(cols = 4:5, names_to = "Ind", values_to = "ID") %>%
  filter(!is.na(ID)) %>%
  mutate(Ind = "Adult") %>%
  distinct() %>%
  arrange(FullID, ID) %>%
  mutate(n_parent = row_number()) %>%
  select(-c(FullID, Ind)) %>%
  group_by(`Nest ID`)# %>% 
  #mutate(n_years = length(unique(NestYear)))

#parents <- split(parents, parents$NestYear) 
#parents <- lapply(parents, pivot_wider, names_from = c(n, NestYear), values_from = ID)

#parents <- parents %>%
 # pivot_wider(names_from = c(n, NestYear), values_from = ID)
