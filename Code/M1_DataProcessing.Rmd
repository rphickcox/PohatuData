...
breeding <- lapply(list.files("./../ALL_DATA/Data", 
                              pattern = "finalsummary_edited.xlsm", 
                              full.names = TRUE,
                              recursive = TRUE, include.dirs = TRUE), 
                   read_excel, col_types = cols)
#names(breeding[[1]]) == names(breeding[[2]])

monitoring_past <- bind_rows(lapply(list.files("./../ALL_DATA/Data", pattern = "Monitoring_20", 
                                               full.names = TRUE,
                                               recursive = TRUE, 
                                               include.dirs = TRUE), 
                                    read_excel))
...
parents_short <- parents %>%
  select(NestYear, FullID, ID) %>%
  mutate(NestID = substr(FullID, 5, nchar(FullID)), 
         AdultRole = NA, .after = NestYear) %>%
  select(-FullID)

# List of birds, with year columns and nest box they bred in
past_parents <- bind_rows(breeding) %>%
  select(NestYear, NestID, Adult1ID, Adult2ID) %>%
  pivot_longer(cols = c(Adult1ID, Adult2ID),
               names_to = "AdultRole",
               values_to = "ID") %>%
  filter(!is.na(ID),  ID != "UM") %>%
  distinct(NestYear, ID, NestID) %>%
  bind_rows(parents %>%
              select(NestYear, FullID, ID) %>%
              mutate(NestID = substr(FullID, 5, nchar(FullID)), 
                     AdultRole = NA, .after = NestYear) %>%
              select(-FullID)) %>%
  group_by(NestYear, ID) %>%
  summarise(NestID = paste(unique(NestID), collapse = ", "), .groups = "drop") %>%
  pivot_wider(names_from = NestYear, values_from = NestID) %>%
  left_join(birdid_all %>% select(`Bird ID`, Sex, Age_current, Age_marking, FullID), by = c("ID" = "Bird ID"))

# Saving to Google Drive...
#gs4_create("Nests_year_summary")
write_sheet(past_parents, ss = gs4_find("Nests_year_summary"), sheet = "Nests_year_summary")

```

```{r}
# Compare parents per year 
...
```{r}
openxlsx::write.xlsx(nests_outcome, paste0("./../ALL_DATA/Data/2025-2026/", "BreedingSeason_", yr, "_finalsummary.xlsx"), 
...
nests_outcome_2025 <- read.xlsx(paste0("./../ALL_DATA/Data/2025-2026/", "BreedingSeason_2025_finalsummary_edited.xlsm"))
nests_outcome_2024 <- read.xlsx(paste0("./../ALL_DATA/Data/2024-2025/", "BreedingSeason_2024_finalsummary_edited.xlsm"))
...
# To get resights for just this year
# resight <- resight %>% filter(NestYear_resight == yr)

# Weird resights (usually juvs hanging with chicks)
...
sum_birdid <- birdid_all %>%
  ...
  summarise(total_marked_year = sum(!`Bird ID` %in% c("P41704", "UM"), na.rm = TRUE), 
            ...
            total_adults_marked = sum(Age_marking %in% c("Adult", "Juvenile"), 
                                      na.rm = TRUE)) %>%
  mutate(
    cumsum_total_marked       = cumsum(total_marked_year),
    cumsum_adults_marked      = cumsum(total_adults_marked),
    total_1yr_marked          = replace_na(lag(total_chicks_marked, 1), 0),
    total_2yr_marked          = replace_na(lag(total_chicks_marked, 2), 0),
    total_3yr_marked          = replace_na(lag(total_chicks_marked, 3), 0),
    # all chicks now >=2yr old (lag cumsum by 1, since 1yr-olds not yet breeding)
    total_breeding_age_marked = cumsum_adults_marked + 
      cumsum(replace_na(lag(total_chicks_marked, 2), 0)),    # add 1yr-olds for moult population
    total_moult_age_marked    = total_breeding_age_marked + total_1yr_marked
  )
...
resight_df <- resight_breeding %>%
  mutate(Sex2 = case_when(
    ...
    .default = "Unknown")) %>%
  group_by(NestYear_resight, Sex2) %>%
  ...
  mutate(.after = "resight_total", 
         ...
         `percent_2yr_breeding` = round((resight_2/lag(total_1yr_marked))*100, 1), 
         # 3 yr old Recruitment rate 
         # 3 year olds breeding from the marked chicks 2 years ago, i.e., cohort
         `percent_3yr_breeding` = round((resight_3/lag(lag(total_1yr_marked)))*100, 1), 
         # Breeding rate
         # proportion of breeding age birds >= 2 observed breeding
...
moult_df <- resight_moult %>%
  mutate(
    Sex2 = case_when(
      ...
      .default = "Unknown"),
    NestID      = sub("^.{4}", "", FullID),
    Age_resight = ifelse(Age_marking == "Adult", "Adult",
                         as.character(NestYear_resight - NestYear))  # years since marking
  ) %>%
  group_by(NestYear_resight) %>%
  summarise(
    resight_1 = sum(Age_resight == "1", na.rm = TRUE),   # 1yr olds resighted
    resight_2 = sum(Age_resight == "2", na.rm = TRUE),   # 2yr olds resighted
    resight_3 = sum(Age_resight == "3", na.rm = TRUE),   # 3yr olds resighted
    resight_breedingage = sum(!Age_resight %in% c("0", "1") & !is.na(Age_resight)),  # >=2yr + adults resighted
    resight_total = n(),                                      # all moulters resighted
    diff_nestbox = sum(NestID != NestID_resight, na.rm = TRUE),  # resighted at different box
    same_nestbox = sum(NestID == NestID_resight, na.rm = TRUE)   # resighted at same box
  ) %>%
  full_join(sum_birdid, by = c("NestYear_resight" = "NestYear")) %>%
  arrange(NestYear_resight) %>%
  relocate(total_marked_year:total_adults_marked, .after = 1) %>%
  mutate( .after = "resight_total",
          # 1yr-olds moulting / chicks marked last year (cohort)
          `percent_1yr_moulting` = round((resight_1 / total_1yr_marked) * 100, 1),
          # 2yr-olds moulting / chicks marked 2 years ago (cohort)
          `percent_2yr_moulting` = round((resight_2 / lag(total_1yr_marked, 1)) * 100, 1),
          # 3yr-olds moulting / chicks marked 3 years ago (cohort)
          `percent_3yr_moulting` = round((resight_3 / lag(total_1yr_marked, 2)) * 100, 1),
          # proportion of breeding age birds >= 2 observed moulting
          `percent_breeding_age_moulting` = round((resight_total / total_breeding_age_marked) * 100, 1),
          # proportion of moult age birds >= 1 observed moulting
          `percent_moulting_age_moulting` = round((resight_total / total_moult_age_marked) * 100, 1)
  )

# Filter birds in the moult dataset but absent from the breeding dataset
x <- resight_moult %>% filter(!BirdID %in% resight_breeding$BirdID)

# Number of resights summary 
resight_summary <- bind_rows(
  resight_weird    %>% mutate(category = "Weird (juv with chicks)"),
  resight_breeding %>% mutate(category = "Breeding/Failed"),
  resight_breeding %>%
    anti_join(resight_moult, by = c("BirdID", "NestYear_resight")) %>%
    mutate(category = "Breeding only"),
  resight_moult %>%
    anti_join(resight_breeding, by = c("BirdID", "NestYear_resight")) %>%
    mutate(category = "Moulting only"),
  resight_moult    %>% mutate(category = "Moulting (all)"),
  resight_loaf     %>% mutate(category = "Loafing only")
) %>%
  group_by(NestYear_resight, category) %>%
...
weights_time <- nests %>%
  ...
  mutate(Chick = paste(FullID, chick_number, sep = "_"),
         Week_f = factor(Week, levels = c(40:58, 1:10)))

# Past years
nests_past <- monitoring_past %>%
  distinct(across(DateTime:`Chick 4 weight`), .keep_all = TRUE) %>%
  mutate(NestYear = ifelse(month(DateTime) > 7, year(DateTime), year(DateTime)-1),
         FullID = paste0(NestYear, `Nest ID`), 
         Eggs = ifelse(is.na(Eggs), 0, as.numeric(Eggs)), 
  ...
  mutate(Nest_number = ifelse(Nest_number == 0, NA, Nest_number), 
         NestID_count = n_distinct(Nest_number, na.rm = TRUE)) %>%  # Count distinct Nest_numbers per FullID
  mutate(FullID = ifelse(NestID_count > 1 & !is.na(NestID_count), paste(FullID, Nest_number, sep = "_"), FullID)) %>%
  select(-c(NewNestFlag, NestID_count, Nest_number)) %>%
  mutate(.after = 2,
         DateTime = as.Date(DateTime),
         Week = ifelse(year(DateTime) > yr, week(DateTime)+52, week(DateTime)),
         Date_last = difftime(as.Date(DateTime), as.Date(Date_last), units = "days"), 
...
breeding_filter <- bind_rows(breeding) %>%
  select(SeasonID, NestYear, Status, HatchDate, Est_FledgeDate, FledgeDate) %>%
  rename("FullID" = "SeasonID")

weights_time_old <- nests_past %>%
  distinct() %>%
  select(NestYear, FullID, DateTime, Week, starts_with("Chick "), Est_LayDate, Est_HatchDate) %>%
  filter(!is.na(`Chick 1 weight`) | !is.na(`Chick 2 weight`)) %>%
  left_join(breeding_filter, by = c("FullID", "NestYear")) %>%
  pivot_longer(
    cols = c("Chick 1 ID", "Chick 1 weight", "Chick 1 status", "Chick 2 ID", "Chick 2 weight", "Chick 2 status"),
    names_to = c("chick_number", ".value"),
    names_pattern = "Chick (\\d+) (.*)") %>%
...
weights_time_total <- bind_rows(weights_time, weights_time_old) %>% 
  distinct() %>%
  arrange(FullID) %>%
  mutate(DateTime = as.POSIXct(DateTime))

# ggplot(data = weights_time_total, aes(x = Week_f, y = weight, color = NestYear, group = FullID)) +
...
weights_age <- weights_time_total %>%
  mutate(Days_old = as.numeric(DateTime - HatchDate),
         Weeks_old = as.numeric(Days_old/7)) %>%
  filter(status != "Uplifted" & Days_old > 0 & Days_old < 100)  %>% # no rehab chicks
...
# weights_summary_nest <- weights_age_sequence %>%
#   group_by(FullID) %>%
#   left_join(birdid_adults[c(2,3,6)], by = "FullID") %>%
#   summarise(Bird_ID = `Bird ID`) %>%
#   distinct()
...
ggplot(data = weights_age_sequence, aes(x = Days_old, y = weight, color = factor(NestYear), group = Chick)) +
  ...
  scale_color_viridis_d("NestYear") +  # Manual color scale for NestYear
  theme_minimal() +
  theme(
    panel.grid.major = element_line(color = "gray80", size = 0.5),  # Lighter grid lines
    panel.grid.minor = element_line(color = "gray90", size = 0.25),
...
ggsave("./../ALL_DATA/Data/Chickweights_byage.png", 
       width = 9, 
       height = 5, bg = "white")
...
ggplot(weights_summary, aes(x = Days_old, y = mean_weight, color = factor(NestYear))) +
  ...
  labs(
    title = "Mean and Standard Error of Chick Weights By Age",
    x = "Days old",
    y = "Weight (grams)"
  ) +
  scale_color_viridis_d("NestYear") +  # Manual color scale for NestYear
  scale_fill_viridis_d("NestYear") +  # Manual color scale for NestYear
  theme_minimal() +
  theme(
...
ggsave("./../ALL_DATA/Data/Chickweights_meanbyage.png", 
       width = 9, 
       height = 5, bg = "white")
