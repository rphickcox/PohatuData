...
nests_outcome <- nests_outcome %>%
  ...
  mutate(EC1Fate = ifelse(Status == "Fledged" & NFledged <= 2, "Fledged", EC1Fate), 
         ...
         Comments = str_squish(Comments)) %>%
  select(NestYear, SeasonID, NestID, starts_with("Adult"), Status, NLaid:diffchicks, NFledged, EC1Fate:EC2Intervention, Chick1ID:Chick3ID, Est_LayDate:FledgeDate, Comments, Nvisits)
```
...
resight <- monitor %>%
  select(DateTime, `Nest ID`, all_of(cc)) %>% 
