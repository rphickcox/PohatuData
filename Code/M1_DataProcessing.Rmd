...
nests <- monitor %>%
  ...
  dplyr::select(NestYear, `Nest ID`, Date_last:Est_HatchDate, `Nest activity`:`Adult 1`, 
...
eggs <- nests %>% 
  ...
  dplyr::select(NestYear, DateTime, `Nest ID`, Status, Est_LayDate:Est_HatchDate, 
                Eggs, Chicks, Notes) %>% 
...
chicks <- nests %>% 
  ...
  dplyr::select(NestYear, DateTime, `Nest ID`, Status, Est_LayDate:UpliftDate, 
                Eggs, Chicks, Notes) %>% 
  distinct(across(c(NestYear, `Nest ID`:Chicks)), .keep_all = T) %>%
  arrange(NestYear, `Nest ID`, DateTime) %>% 
...
# failed_chick <- chicks %>% 
#   filter(Status == "Failed") %>%
#   filter(Chicks_1 != 0 & !is.na(Chicks_1)) %>%
# mutate(across(Est_TranspondDate5:Est_FledgeDate, ~NA))
# 
# 
# chicks <- chicks %>% 
...
### Eggs + Chicks + Totals
```{r ect}
#allfailed <- rbind(failed_eggs, failed_chick)

combo_nests <- eggs %>% 
  full_join(chicks[c(1:2, 5, 9:16)], by = c("Nest ID", "NestYear"), suffix = c("e", "")) %>% 
  group_by(NestYear, `Nest ID`) %>% 
  ...
  dplyr::select(c(NestYear, `Nest ID`, Status, Est_LayDate, Est_HatchDate, HatchDate, UpliftDate, Est_FledgeDate, FledgeDate, Eggs_1, Eggs_2, NLaid, diffeggs, Chicks_1, Chicks_2, NHatched, diffchicks, Comments))

# TOTALS
# MAJOR ISSUES HERE! WRONG NUMBERS BECAUSE OF YEARS! PERHAPS NEED TO INDEX
fn_year <- failed_nests %>% group_by(NestYear) %>% summarise(Nests_failed = n())
fn_uplifted <- combo_nests[combo_nests$Status == "Uplifted",] %>% group_by(NestYear) %>% summarise(Nests_uplifted = n())
fn_year <- left_join(fn_year, fn_uplifted, by = "NestYear")

totals <- combo_nests %>% 
  ungroup() %>%
  group_by(NestYear) %>%
  summarise(Total_nests = length(unique(`Nest ID`)), 
           # Nests_failed = fn_year$Nests_failed,
