...
# SETUP
```{r}
date_before <- as.Date("2022-11-01")
...
archive.photo <-  function(ls_down, to_download, save_to){
  newpath <- filter(to_download, Image.http == ls_down)
  newpath <- newpath$newpath
  folder <- str_split(newpath, "\\W")[[1]][1]
  ifelse(!dir.exists(file.path(save_to, folder)), dir.create(file.path(save_to, folder)), FALSE)
  drive_download(file = ls_down, 
                 overwrite = TRUE, 
...
to_download <- monitor %>%
  ...
  filter(!is.na(Image), pathname %in% files_drive$name)
...
lapply(ls_down, FUN = archive.photo, to_download = to_download, save_to = save_to) 
```

# ECECUTING DELETE FUNCTION
```{r}
# Delete image
